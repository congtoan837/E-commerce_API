...
public interface OrderRepository extends CrudRepository<Orders, Integer> {
    @Query("SELECT NEW com.poly.model.OrderDTO(o.Id, o.users,o.Address,o.promotion,o.payment, o.Status, (SELECT SUM(pd.Price*oa.Quantity)-SUM(pd.Price*oa.Quantity)*(pm.percents/100) from OrderDetails oa inner join Product pd on pd.Id = oa.product.Id where oa.orders.Id = o.Id)) FROM Orders o inner join Users u on o.users.Id = u.Id inner join Payment p on o.payment.Id = p.Id inner join Promotion pm on pm.Coupon = o.promotion.Coupon where o.users.Id = :id")
