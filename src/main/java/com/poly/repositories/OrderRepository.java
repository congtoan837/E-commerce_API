...
public interface OrderRepository extends CrudRepository<Orders, Integer> {
    @Query("SELECT NEW com.poly.model.OrderDTO(o.Id, u.Name, u.Phone,o.Address,pm.percents,o.Promotion,p.Name, o.Status, (SELECT SUM(pd.Price*oa.Quantity)-SUM(pd.Price*oa.Quantity)*(pm.percents/100) from OrderDetails oa inner join Product pd on pd.Id = oa.product.Id where oa.orders.Id = o.Id)) FROM Orders o inner join Users u on o.CustomerId = u.Id inner join Payment p on o.PaymentId = p.Id inner join Promotion pm on pm.Coupon = o.Promotion where o.CustomerId = :id")
